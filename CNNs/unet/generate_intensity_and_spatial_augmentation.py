...
class ReplaceNaNAndInf(tio.Transform):
    """Custom transform to replace NaN and Inf values with 0."""
    def apply_transform(self, subject):
        image = subject['mri']
        data = image.data
        data[~torch.isfinite(data)] = 0  # set NaN or Inf to 0
        image.set_data(data)
        return subject
...
outputPath = '/home/martin/data_imaging/Muscle/GlutealSegmentations/PelvisFOV/TrainingSetFromManual/IntensityAndSpatiallyAugmentedDownsampled/' # Base data path.
dataPath = "/home/martin/data_imaging/Muscle/LumbarSpine/ManualSegmentations/MhdRegisteredDownsampled/"
outputPath = "/home/martin/data_imaging/Muscle/LumbarSpine/ManualSegmentations/TrainingSetFromManual/IntensityAndSpatiallyAugmentedDownsampled/"
...
transform = tio.Compose([
    tio.RandomAffine(
        scales=(0.9, 1.1),  # isotropic scaling
        degrees=(2, 2, 10),  # random rotations up to ±10°
        translation=10,  # up to ±5 voxels
        p=0.8
    ),
...
for i in range(0, len(atlasNames)):
    ...
    label_array = sitk.GetArrayFromImage(label)  # shape: [Z, Y, X]
    # TorchIO expects [C, Z, Y, X]
    image_tensor = image_array[np.newaxis, ...]  # add channel axis
    label_tensor = label_array[np.newaxis, ...]  # add channel axis
    ...
    for j in range(10):
        ...
        augmented_label_array = augmented.label.data.squeeze().numpy()
        ...
        augmented_label = sitk.GetImageFromArray(augmented_label_array)
        augmented_label.CopyInformation(label)

        print(f"Min: {augmented_array.min()}, Max: {augmented_array.max()}")
        print(f"Min: {augmented_label_array.min()}, Max: {augmented_label_array.max()}")
        # write the  images:
        sitk.WriteImage(augmented_image, outputPath + atlasNames[i] + '_aug' + str(j) +'.' + extensionImages, True)
        sitk.WriteImage(augmented_label, outputPath + atlasNames[i] + '_aug' + str(j)  + tagManLabels + '.' + extensionImages, True)
