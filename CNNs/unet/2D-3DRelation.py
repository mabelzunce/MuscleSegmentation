...
dataPath ="../../Data/ResultadosAntropometricosCSV/"
Matrix = []
...
ff2D = np.transpose(df2D[muscleHeaders].values)
ff3D = np.transpose(df3D[muscleHeaders].values)
def format_2_decimal_places(x, pos):
    return f'{x:.2f}'
for f2, f3, names in zip(ff2D,ff3D, nombreMusculos):
    ...
    plt.title('Fracción de tejido graso' + ' ' + names)
    ...
    plt.savefig(dataPath + 'RL_FatFraction' + '_' + names +'.tif')
    plt.close()
