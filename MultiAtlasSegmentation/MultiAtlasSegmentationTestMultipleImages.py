...
############################### TARGET FOLDER ###################################
targetPath = "D:\\Martin\\Segmentation\\AtlasLibrary\\RawImagesTrimmed\\"
...
###################### OUTPUT #####################
# Output path:
baseOutputPath = "D:\\MuscleSegmentationEvaluation\\SegmentationWithPython\\FullTest\\"
...
for targetFilename in targetImagesNames:
    ...
    for filename in files:
        ...
        #    # Use only the marathon study
        #    if str(name).startswith("ID"):
        if str(extension).endswith(extensionImages) and not str(name).endswith('labels') and not name.startswith(
                nameCaseFixed):
            # Intensity image:
